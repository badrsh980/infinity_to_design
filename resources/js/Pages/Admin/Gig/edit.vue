...
export default {
    ...
    methods: {
        update() {
            this.form.post(route("admin.gigs.update"), {
                preserveScroll: true,
                onError: (errors) => {
                    console.log(errors);
                },
    ...
    mounted() {
        this.form.status = this.gig.status;
        this.form.title = this.gig.title;
        this.form.slug = this.gig.slug;
        this.form.description = this.gig.description;
        this.form.thumbnails = this.gig.thumbnails;
        this.form.delivery_time = this.gig.delivery_time;
        this.form.instructions = this.gig.instructions;
        this.form.price = this.gig.price;
        this.form.user_changes = this.gig.user_changes;
        this.form.language = this.gig.language;
        this.form.user_id = this.gig.user_id;
        this.form.category_id = this.gig.category_id;
    },
};
</script>
    ...
    <AdminLayout>
        <div class="w-3/4 mx-auto p-4 bg-white">
            <form @submit.prevent="update">
                ...
                <div class="mb-4">
                    <InputLabel for='slug' :value="$t('message.slug')" />
                ...
                <div class="mb-4">
                    <InputLabel for='thumbnails'
                    :value="$t('message.thumbnails')" />
                    ...
                    <InputError
                        class="mt-2"
                        :message="form.errors.thumbnails"
                    />
                </div>
                ...
                <div class="mb-4">
                    <InputLabel for='user_changes'
                    :value="$t('message.user_changes')" />

                    <textarea
                        id="user_changes"
                        class="border-gray-500 rounded-sm shadow-sm w-full"
                        v-model="form.user_changes"
                    ></textarea>

                    <InputError
                        class="mt-2"
                        :message="form.errors.user_changes"
                    />
                </div>

                <div class="mb-4">
                    <InputLabel for='language' :value="$t('message.language')"
                    />
...
export default {
    ...
    methods: {
        update() {
            ...
            this.form.post(route("admin.gigs.update", { gig: this.gig.id }), {
                preserveScroll: true,
                onError: (errors) => {
                    console.log(errors);
                },
    ...
    computed: {
        cextras() {
            const results = this.form.extras.map((extra, index) => {
                this.extras[index] = {
        ...
        cthumbnails() {
            const results = this.form.thumbnails.map((image) => {
                if (typeof image == "string") {
                    return "/storage/images/gigs/" + image;
                }

                return this.url.createObjectURL(image);
            });
            return results;
        },
    },
    mounted() {
        ...
        this.form.user = this.gig.user.name;
        ...
        <div class="w-full md:w-3/4 mx-auto p-4 bg-white">
            ...
            >
                ...
                <div class="mb-4 col-span-2">
                    ...
                    >
                        <div
                            v-for="(image, index) in cthumbnails"
                            class="relative"
                        >
                            <button
                                type="button"
                            ...
                            <img class="h-40 w-full rounded-lg" :src="image" />
                        </div>
                    </div>
                ...
                <div class="mb-4 col-span-2" v-if="form.extras.length > 0">
                    ...
                    >
                        ...
                        <div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div v-if="extras[index]?.change_gig_date == 'yes'">
                                <select
                                    :id="'etime-' + index"
                                    class="border-gray-500 rounded-sm shadow-sm w-full"
                                    v-model="extra.time"
